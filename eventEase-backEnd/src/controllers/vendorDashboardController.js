  ...
  try {
    ...
    const teamUtilization = teamMembers.length > 0 ? teamMembers.map(member => {
      const memberEvents = events.filter(event => {
        // This would need to be tracked in a separate table in a real implementation
      ...
      return {
        ...
        eventsAssigned: teamMembers.length > 0 ? Math.floor(memberEvents.length / teamMembers.length) : 0 // Rough estimate
      };
    }) : [];
